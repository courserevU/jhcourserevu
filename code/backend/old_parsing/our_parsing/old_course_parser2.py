...
class Parser(BaseParser):
    ...
    def _compute_size_enrollment(self, course):
        try:
            ...
            section_size = 0
        try:
            section_enrolment = section_size - int(
                course["SeatsAvailable"].split("/")[0]
            )
    ...
    def _load_ingestor(self, school, course, section):
        ...
        if (
            ...
            or school == "Whiting School of Engineering"
        ):
        ...
        self.ingestor["level"] = (
            re.findall(re.compile(r".+?\..+?\.(.{1}).+"), course["OfferingName"])[0]
            + "00"
        )
        ...
        for meeting in section_details[0]["Meetings"]:
            ...
            self.ingestor["instrs"] = [
                i.strip() for i in course["Instructors"].split(",")
            ]
            ...
            for time in [t for t in times.split(",") if len(t) > 0]:
                time_pieces = re.search(
                    r"(\d{2}:\d{2} [AP]M) - (\d{2}:\d{2} [AP]M)", time
                )
                ...
                if (
                    len(meeting["DOW"].strip()) > 0
                    and meeting["DOW"] != "TBA"
                    and meeting["DOW"] != "None"
                ):
                    self.ingestor["days"] = [
                        Parser.DAY_MAP.get(d.lower())
                        for d in re.findall(
                            r"(?:T[hH])|(?:S[aA])|[SMTWF]", meeting["DOW"]
                        )
                    ]
    ...
    def start(
        ...
        years_and_terms = dict_filter_by_dict(
            {year: [term for term in terms] for year in years}, years_and_terms_filter
        )
